...
def gini_coefficient(network):
    ...
    sumList = []
    equal = [0]
    # This is just a helper-loop in order to get the
    # Line of Equality
    ...
    for i in range(0,len(network)):
        sum += network[i]
        sumList.append(sum)
    # We calculate the Gini coefficient as: G = A / O where
    # A is the area of the Line of equality minus the area
    # of the Lorenz Curve and O the total area beneath the Line
    # of equality. The area beneath the lines are calculated
    # by using the integrate function of scipy
    ...
    for cust in range(2, customers+1):
            ...
            for table, guests in enumerate(tables):
                ...
                if rand < prob:
                    ...
                    table_found = True
                    # no more tables need to be iterated,
                    # break out for loop
                    break
            # If table iteration is over and no table was found,
            # open new table
            if not table_found:
                tables.append(1)
    return tables
